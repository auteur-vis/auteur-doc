...
export default function Vis({size={"width":500, "height":500}, sparse=false}) {
	...
	useEffect(() => {
		...
		if (sparse) {
			draft.current.exclude({"name":["label", "regression", "text"]}).augment(newEmphasis.current.intersect(newCatEmphasis.current));
		} else {
			draft.current.augment(newEmphasis.current.intersect(newCatEmphasis.current));
		}
	...
	return (
		<div>
			{sparse
				? <h3><a href="./gallery/EmphasisScatterCompound">intersection of emphases</a></h3>
