...
export default function Vis({size={"width":900, "height":500}, sparse=false}) {
	...
	useEffect(() => {
		...
		if (sparse) {
			draft.current.exclude({"name":["label", "regression", "text"]}).augment(newXThreshold.current.getAugs());
		} else {
			draft.current.augment(newXThreshold.current.getAugs());
		}
	...
	return (
		<div>
			{sparse
				? <h3><a href="./gallery/ThresholdLineBasic">line chart</a></h3>
				: <div />
			}
			<svg id="less" ref={ref}>
				<g id="mark" />
